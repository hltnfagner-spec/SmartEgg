-- ========================================
-- SCRIPT DE CORREÇÃO DE RECURSÃO INFINITA
-- Execute ESTE script primeiro para limpar políticas antigas
-- ========================================
...
-- 3. NOVA política SEM recursão para admin_users
CREATE POLICY "Public read access to admin_users"
  ON admin_users FOR SELECT
  USING (true);

-- 4. NOVAS políticas para subscriptions (SEM recursão)
...
-- 5. NOVAS políticas para company_settings (SEM recursão)
CREATE POLICY "Admins view all company settings"
...
-- 6. Tornar AMBOS os emails administradores
INSERT INTO admin_users (user_id)
SELECT id FROM auth.users 
WHERE email IN ('hltnfagner@gmail.com', 'hiltonfag@gmail.com')
ON CONFLICT (user_id) DO NOTHING;

-- 7. Ativar assinaturas PERPÉTUAS (100 anos)
...
SELECT 
  u.email,
  CASE WHEN a.id IS NOT NULL THEN '✅ ADMIN FULL' ELSE '❌ Normal' END as "Permissão",
...
-- 9. Testar se admin_users funciona sem recursão
